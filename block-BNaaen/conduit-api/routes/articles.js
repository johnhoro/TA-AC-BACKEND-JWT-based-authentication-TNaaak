    ...
    if (user) {
      ...
      res.json({
        articles: articles.map((article) =>
          articleGenerator(article, user, req.body.user.userId)
        ),
      });
    } else {
...
router.post("/", auth.verifyToken, async (req, res) => {
  try {
    // req.body.article.author = req.body.user.userId;
...
router.put("/:slug", auth.verifyToken, async (req, res) => {
  try {
    ...
    if (article.author.toString() === req.body.user.userId) {
      if (req.body.article.title) {
        req.body.article.slug = auth.slug(req.body.article.title);
        console.log(req.body.article.slug);
      }
      console.log(req.body.article, req.params.slug);
      ...
      console.log(updateArticle, `jjoohhnn`);
...
router.delete("/:slug", auth.verifyToken, async (req, res) => {
  try {
    const article = await Article.findOne({ slug: req.params.slug });
    console.log(article);
    if (article.author.toString() === req.body.user.userId.toString()) {
